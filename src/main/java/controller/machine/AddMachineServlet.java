...
@WebServlet(name = "AddMachineServlet", urlPatterns = {"/mgr/add-machine"})
public class AddMachineServlet extends HttpServlet {

    private static final String ADD_MACHINE_PAGE = "/mgr/machine/add-machine.jsp";
    ...
    private void handleAddSingleMachine(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (newUnitId > 0) {
                response.sendRedirect(request.getContextPath() + MACHINE_LIST_URL + "?added=true");
            } else {
